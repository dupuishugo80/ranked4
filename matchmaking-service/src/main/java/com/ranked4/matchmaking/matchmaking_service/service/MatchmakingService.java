...
public class MatchmakingService {
    ...
    public void joinQueue(UUID userId) {
        fetchUserProfile(userId)
            .flatMap(profile -> {
                ...
                return findAndProcessMatch(userId, profile.getElo())
                    .switchIfEmpty(Mono.defer(() -> { 
                        log.info("No match found for {}. Adding to queue.", userId);
                        return addUserToQueue(userId, profile.getElo())
                            .thenReturn(false);
                    }));
            })
            .subscribe(
    ...
    private Mono<Boolean> findAndProcessMatch(UUID searchingPlayerId, int searchingPlayerElo) {
        double minElo = searchingPlayerElo - ELO_RANGE;
        double maxElo = searchingPlayerElo + ELO_RANGE;
    ...
    private Mono<Void> addUserToQueue(UUID userId, int elo) {
        log.info("Adding {} to queue (ELO: {})", userId, elo);
        zSetOps.add(MATCHMAKING_QUEUE_KEY, userId.toString(), elo);
        return Mono.empty();
    }
}